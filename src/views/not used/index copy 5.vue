...
export default {
  ...
  setup() {
    ...
    const generateGrid = () => {
      ...
      // Add tile coordinates to each cell
    ...
    // Add tile coordinates to each cell (AA-34, AA-33, etc.)
    const addTileCoordinatesToCells = () => {
      ...
      // Sort columns by X position and rows by Y position
      const sortedColumns = [...columns].sort((a, b) => a.x - b.x)
      const sortedRows = [...rows].sort((a, b) => a.y - b.y)
      
      // Generate tile coordinates for each cell
      sortedColumns.forEach((col, colIndex) => {
        sortedRows.forEach((row, rowIndex) => {
          // Use the actual row label from the SVG (e.g., 34, 33, 32, etc.)
          const rowNumber = row.label.padStart(2, '0')
          const tileCoordinate = `${col.label}-${rowNumber}`
          
          // Calculate cell center position
          const cellCenterX = col.x
          const cellCenterY = row.y
          
          // Add tile coordinate text to the cell
          gridLayer.text(tileCoordinate)
            .move(cellCenterX - 15, cellCenterY - 6)
            .font({ family: 'Arial', size: 30, weight: 'normal' })
            .fill('#999')
      ...
      console.log('Tile coordinates added to grid cells')
    }
